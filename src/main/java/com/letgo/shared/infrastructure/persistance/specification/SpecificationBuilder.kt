...
class SpecificationBuilder(strategies: List<SpecificationStrategies>) {
    private val specificationStrategies = strategies.reduce { current, next ->
    ...
    private fun findStrategy(filter: Filter): (Any, AggregateRoot) -> Boolean {
        return specificationStrategies.map[filter.name]?.get(filter.operator) ?: throw NoSuchElementException(
